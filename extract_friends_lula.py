...
d = {'LulaOficial_following': ['dummy'], 'followers': '0'}
...
for user in tweepy.Cursor(api.friends, screen_name="LulaOficial").items():
    print('friend: ' + user.screen_name)
    df = df.append({'twitter_user': f"{user.screen_name}", 'followers': f"{user.followers_count}"}, ignore_index=True)

df.to_csv('LulaOficial_following.csv', index=False)
df.to_excel('LulaOficial_following.xlsx', index=False)
